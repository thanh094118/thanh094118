...
contract UnstoppableChallenge is Test {
    ...
    /*
    Lỗ hổng:
    Vault dùng token.balanceOf(address(this)) làm totalAssets, với token chỉ thay đổi qua deposit(). 
    Ta có thể transfer() token vào vault, khiến totalAssets tăng lệch so với totalSupply, phá invariant.
    Crack:
    Gửi thêm 1 token trực tiếp vào vault
    Sẽ làm sai lệch cân bằng asset/share, khiến flash loan của vault luôn fail. Khi đó vault trở nên “unstoppable”.
     */
